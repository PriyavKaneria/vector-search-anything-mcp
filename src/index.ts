...
class TextSearchServer {
  ...
  async addText(text: string): Promise<number> {
    ...
    const existingText = this.db.prepare(
      "SELECT id FROM text_chunks WHERE text = ?"
    ).get(cleanText);

    if (existingText) {
      console.error(`Text already exists with ID: ${(existingText as any).id}`);
      return (existingText as any).id;
    }
    ...
    const insertStmt = this.db.prepare(
      "INSERT INTO text_chunks (text, embedding) VALUES (?, ?)"
    );
  ...
  async loadSpreadsheet(filePath: string, clearExisting: boolean = true) {
  ...
  async searchSimilar(query: string, topN: number = 5) {
    await this.initializeEmbedder();
    ...
    const allChunks = this.db.prepare(
      "SELECT id, text, embedding FROM text_chunks WHERE embedding IS NOT NULL"
    ).all();

    const similarities: Array<{ id: number; text: string; similarity: number }> = [];
  ...
  try {
    switch (name) {
      case "load_spreadsheet": {
        const { file_path, clear_existing = true } = args as { file_path: string; clear_existing?: boolean };
      ...
      case "search_text": {
        const { query, top_n = 5 } = args as { query: string; top_n?: number };
        const results = await searchServer.searchSimilar(query, top_n);
        
        const formattedResults = results.map((result, index) => 
          `${index + 1}. (Similarity: ${result.similarity.toFixed(4)}) ${result.text}`
        ).join('\n\n');
